...
async def test_startup_event(mock_sync, mock_upgrade):
    """Verify that startup event successfully creates tables, runs migrations, and triggers sync."""
    async with lifespan(app):
        pass
    # # mock_create.assert_called_once()  # Removed in codebase
...
async def test_startup_event_exceptions(mock_sync, mock_upgrade, capfd, caplog):
