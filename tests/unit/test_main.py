...
def test_update_settings_new_config():
    ...
    response = client.post(
        ...
        data={
            ...
            "enabled": "1",
            "target_url": "http://makerworld.com/my-target",
...
def test_browse_directories_success():
    ...
    with patch("os.scandir") as mock_scandir:
        ...
        # assert '..' in response.text  # Removed because /mock is resolved outside allowed base /
    ...
    with patch("src.app.main.run_scraper", return_value=[]) as mock_fetch:
        response = client.post(
            ...
            data={
                ...
                "target_url": "http://minihoarder.com/my-target",
                "credential": "",
            },
        )
        assert response.status_code == 200
        mock_fetch.assert_called_once_with("minihoarder", "http://minihoarder.com/my-target")
    db.close()
...
def test_update_settings_existing_config():
    ...
    assert getattr(config, "target_url") == "http://makerworld.com/my-target"
