"""Test module for the Temporal workflows and activities."""
...
@pytest.mark.asyncio
@patch("src.worker.temporal_workflows.run_scraper")
async def test_sync_makerworld(mock_run_scraper):
    """Verify that the MakerWorld task triggers the scraper with the correct target."""
    mock_run_scraper.return_value = [{"title": "Test"}]
    result = await sync_makerworld()
...
@pytest.mark.asyncio
@patch("src.worker.temporal_workflows.fetch_thingiverse_collections")
async def test_sync_thingiverse_api_success(mock_fetch_api):
...
@patch("src.worker.temporal_workflows.run_scraper")
@patch("src.worker.temporal_workflows.fetch_thingiverse_collections")
async def test_sync_thingiverse_api_fallback(mock_fetch_api, mock_run_scraper):
...
@pytest.mark.asyncio
@patch("src.worker.temporal_workflows.run_scraper")
async def test_sync_minihoarder(mock_run_scraper):
    """Verify that the Minihoarder task triggers the scraper with the correct target."""
    mock_run_scraper.return_value = [{"title": "Test"}]
    result = await sync_minihoarder()
    mock_run_scraper.assert_called_once_with("minihoarder", "https://www.minihoarder.com/library/")
    assert result == [{"title": "Test"}]


@pytest.mark.asyncio
@patch("src.worker.temporal_workflows.settings")
@patch("src.worker.temporal_workflows.SessionLocal")
async def test_sync_local(mock_session, mock_settings, tmp_path):
