...
def test_fetch_thingiverse_no_token():
    """
...
def test_fetch_thingiverse_success(mock_get):
    """
    ...
    result = fetch_thingiverse_collections("fake_token")
...
@patch("src.worker.thingiverse_api.httpx.get")
def test_fetch_thingiverse_existing_item(mock_get):
    """
    ...
    # First call adds it
    fetch_thingiverse_collections("fake_token")
    # Second call should ignore it and return nothing saved
    result2 = fetch_thingiverse_collections("fake_token")

    assert not result2


@patch("src.worker.thingiverse_api.httpx.get")
def test_fetch_thingiverse_http_error(mock_get):
    """
...
@patch("src.worker.thingiverse_api.httpx.get")
def test_fetch_thingiverse_db_error(mock_get):
    """
