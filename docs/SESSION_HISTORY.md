...
* **Follow-up Instruction**: Implement actual live retrieval capabilities.
  * *Implementation*: Added Playwright to fetch dynamic HTML pages while injecting user session cookies.
  * *Implementation*: Wrote a specific `httpx` integration for the official Thingiverse API that bypasses the LLM when an API token is provided.
