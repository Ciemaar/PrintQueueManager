...
If you are new to background tasks, please read the [RQ & Redis Tutorial](RQ_REDIS_TUTORIAL.md) for an explanation of the asynchronous scraping architecture.

- **RQ & Redis:** To execute asynchronous tasks in the background, like web scraping and syncing models from online sources.
...
3. In `src/worker/rq_worker.py`, define a new standard Python function to fetch the target HTML and execute the LLM scraper, then set up `rq-scheduler` scheduling in `src/worker/scheduler.py` to call your function periodically.
