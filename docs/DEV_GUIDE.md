...
We rely on `poethepoet` for automation. Running `uv run poe check` will check the code quality across the entire project.

- `uv run poe pyright`: Runs Pyright type checking.
- `uv run poe ruff`: Runs Ruff linter and formatter.
- `uv run poe test`: Runs unit and integration tests with `hypothesis` for property-based generation and testing of schema components.
