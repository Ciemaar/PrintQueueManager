...
If you are new to background tasks, please read the [Dramatiq & Redis Tutorial](DRAMATIQ_REDIS_TUTORIAL.md) for an explanation of the asynchronous scraping architecture.

- **Dramatiq & Redis:** To execute asynchronous tasks in the background, like web scraping and syncing models from online sources.
...
Ensure you have Python 3.14+ and a local PostgreSQL + Redis instance running. We recommend using `docker compose` to spin up your databases and then running your web/worker/watchdog locally using [uv](https://docs.astral.sh/uv/) for incredibly fast dependency management. (See the [Docker Compose Guide](DOCKER_GUIDE.md) for detailed container instructions.)
...
3. In `src/worker/dramatiq_app.py`, define a new Dramatiq task (`@dramatiq_app.task`) to fetch the target HTML and execute the LLM scraper, then set up Dramatiq beat scheduling to call your function periodically.
