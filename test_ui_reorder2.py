...
os.environ["DATABASE_URL"] = "sqlite:///./test_ui_reorder2.db"
...
engine = create_engine("sqlite:///./test_ui_reorder2.db", connect_args={"check_same_thread": False})
...
def setup_db():
    ...
    # WHAT IF priorities are NORMALIZED already?
    # Or what if they are randomly set? Let's use integers to mimic normal state.
    job1 = PrintJob(title="Job A (1)", source="Test", user_priority=1.0)
    db.add(job1)
    db.commit()
    time.sleep(0.1)
    job2 = PrintJob(title="Job B (2)", source="Test", user_priority=2.0)
    db.add(job2)
    db.commit()
    time.sleep(0.1)
    job3 = PrintJob(title="Job C (3)", source="Test", user_priority=3.0)
    db.add(job3)
    db.commit()
    db.close()
...
def verify_ui():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Initial order: Job A, B, C (because 1.0, 2.0, 3.0)
            print("Initial UI order:")
            for i in range(3):
                print(f"{i}: {rows.nth(i).locator('strong').inner_text()}")

            # Let's drag the second item (Job B) to the top (above Job A)
