    ...
    try:
        if service_name == "local":
            ...
            if parsed_url.scheme not in ("http", "https") or parsed_url.netloc not in (
                ...
                return HTMLResponse(
                    '<div style="color: var(--pico-del-color);">Test failed: Invalid URL for Thingiverse</div><span id="status-indicator-thingiverse" hx-swap-oob="true">❌</span>'  # noqa: E501
                )

            # Simple timeout to prevent hanging the worker
            ...
            allowed_domains = {
                "thingiverse": ["thingiverse.com", "www.thingiverse.com", "api.thingiverse.com"],
                "makerworld": ["makerworld.com", "www.makerworld.com"],
                "printables": ["printables.com", "www.printables.com"],
                "cults3d": ["cults3d.com", "www.cults3d.com"],
                "minihoarder": ["minihoarder.com", "www.minihoarder.com"],
                "myminifactory": ["myminifactory.com", "www.myminifactory.com"],
            }

            if parsed_url.scheme not in ("http", "https"):
                return HTMLResponse(
                    '<div style="color: var(--pico-del-color);">'
                    "Test failed: Invalid URL scheme. Only http/https allowed.</div>"
                )
...
def browse_directories(path: str = "/") -> HTMLResponse:
    ...
    try:
        ...
        if not (path == allowed_base_dir or path.startswith(allowed_base_dir + os.sep)):
            path = allowed_base_dir

        parent_dir = os.path.dirname(path) if path != allowed_base_dir else None
